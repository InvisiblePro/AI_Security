...
recognizer = cv2.face.LBPHFaceRecognizer_create() # using cv2 module for face recognition 
recognizer.read('model/model.yml') # importing trained model
cascadePath = 'haarcascade_frontalface_default.xml' # getting xml data to locate face on camera.
faceCascade = cv2.CascadeClassifier(cascadePath) # getting xml data to locate face on camera.
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        cv2.putText(img, str(accuracy), (x+5, y+h-5), cv2.FONT_HERSHEY_SIMPLEX,1, (255, 255, 0), 2)
    
    
    cv2.putText(img,str(datetime.now().date()), (10,20),cv2.FONT_HERSHEY_SIMPLEX,0.8,(255,255,255),2)
    cv2.putText(img,str(f'{datetime.now().hour}:{datetime.now().minute}:{datetime.now().second}'),(10,50),cv2.FONT_HERSHEY_SIMPLEX,0.8,(255,255,255),2)
    out.write(img)
